...
struct SegmentTree {
	...
	void update(ll x, int idx, int pos, int l, int r) {
		if(l == r) {
			seg[pos] += x;
			return;
		}
		...
		int mid = (l + r) / 2;
		if(idx <= mid) update(x,idx,2*pos+1,l,mid);
		else update(x,idx,2*pos+2,mid+1,r);

		seg[pos] = seg[2*pos+1] + seg[2*pos+2];		
	}

	void update(ll num, int idx) {
		update(num,idx,0,0,n-1);
	}
